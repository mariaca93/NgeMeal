    ...
    public function paymentmethodlist(Request $request)
    {
        ...
        if(!empty($checkuser)){
            if($request->type == "order"){
                $pmdata = Payment::select('id','environment','payment_name','currency','public_key','encryption_key',DB::raw("CONCAT('".url('/storage/app/public/admin-assets/images/about/')."/', image) AS image"))->where('is_available',1)->orderBy('id')->get();
            }
            if($request->type == "wallet"){
                $pmdata = Payment::select('id','environment','payment_name','currency','public_key','encryption_key',DB::raw("CONCAT('".url('/storage/app/public/admin-assets/images/about/')."/', image) AS image"))->where('is_available',1)->where('payment_name','!=','cod')->where('payment_name','!=','wallet')->orderBy('id')->get();
            }
