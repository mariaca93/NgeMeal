...
class HomeController extends Controller
{
    public function newhome(Request $request )
    {
        ...
        $bannerlist = Banner::with('item_info','cuisine_info')->where('is_available',1)->get();
        $banners = array();
        ...
        $cuisinedata = Cuisine::select('id','cuisine_name',DB::raw("CONCAT('".url('/storage/app/public/admin-assets/images/cuisine/')."/', image) AS image"))->where('is_available','=','1')->where('is_deleted','2')->get();
        ...
        $featured = Item::with('cuisine_info','subcuisine_info','variation','item_image')->select('item.*',DB::raw('(case when favorite.item_id is null then 0 else 1 end) as is_favorite'),DB::raw('(case when item.price is null then 0 else item.price end) as price'),DB::raw('(case when cart.item_id is null then 0 else 1 end) as is_cart'))
        ...
        $recommended = Item::with('cuisine_info','subcuisine_info','variation','item_image')->select('item.*',DB::raw('(case when favorite.item_id is null then 0 else 1 end) as is_favorite'),DB::raw('(case when item.price is null then 0 else item.price end) as price'),DB::raw('(case when cart.item_id is null then 0 else 1 end) as is_cart'))
            ->leftJoin('favorite', function($query) use($user_id) {
                $query->on('favorite.item_id','=','item.id')
                ->where('favorite.user_id', '=', $user_id);
            })
