...
class ItemController extends Controller
{
    ...
    public function itemdetails(Request $request)
    {
        ...
        $itemdata  = Item::with('cuisine_info','subcuisine_info','variation','item_images')->select('item.*',DB::raw('(case when favorite.item_id is null then 0 else 1 end) as is_favorite'),DB::raw('(case when item.price is null then 0 else item.price end) as price'),DB::raw('(case when cart.item_id is null then 0 else 1 end) as is_cart'))
            ->leftJoin('favorite', function($query) use($user_id) {
                $query->on('favorite.item_id','=','item.id')
                ->where('favorite.user_id', '=', $user_id);
            })
        ...
        if(!empty($itemdata)){
            ...
            $itemdetails = array(
                ...
                "cuisine_info" => $itemdata->cuisine_info,
                "subcuisine_info" => $itemdata->subcuisine_info,
            ...
            $itemlist = Item::with('cuisine_info','subcuisine_info','variation','item_image')->select('item.*',DB::raw('(case when favorite.item_id is null then 0 else 1 end) as is_favorite'),DB::raw('(case when item.price is null then 0 else item.price end) as price'),DB::raw('(case when cart.item_id is null then 0 else 1 end) as is_cart'))
                ->leftJoin('favorite', function($query) use($user_id) {
                    $query->on('favorite.item_id','=','item.id')
                    ->where('favorite.user_id', '=', $user_id);
                })
                ...
                ->groupBy('item.id','cart.item_id')
                ->where('item.item_status','1')->where('item.is_deleted','2')->where('item.cuisine_id',$itemdata->cuisine_id)->where('item.id','!=',$itemdata->id)
    ...
    public function cuisine_items(Request $request)
    {
        $user_id  = $request->user_id;
        if($request->cuisine_id == ""){
            return response()->json(["status"=>0,"message"=>trans('messages.cuisine_required')],200);
        }
        $checkcuisine = Cuisine::where('id',$request->cuisine_id)->where('is_available',1)->where('is_deleted',2)->first();
        if(!empty($checkcuisine)){
            $data = array();
            $itemlist = Item::with('cuisine_info','subcuisine_info','variation','item_image')->select('item.*',DB::raw('(case when favorite.item_id is null then 0 else 1 end) as is_favorite'),DB::raw('(case when item.price is null then 0 else item.price end) as price'),DB::raw('(case when cart.item_id is null then 0 else 1 end) as is_cart'))
                    ->leftJoin('favorite', function($query) use($user_id) {
                        $query->on('favorite.item_id','=','item.id')
                    ...
                    ->groupBy('item.id','cart.item_id')
                    ->where('item.item_status','1')->where('item.is_deleted','2')->where('item.cuisine_id',$request->cuisine_id)
                    ->orderByDesc('item.id')->get();
            $allitems = array();
            foreach ($itemlist as $item) {
            ...
            $data[] = array(
                "id"=>"",
                "cuisine_id"=>"",
                "subcuisine_name"=>trans('labels.all'),
                "subcuisine_items"=>$allitems,
            );
            $subcuisines = Subcuisine::where('cuisine_id',$request->cuisine_id)->where('is_available',1)->where('is_deleted',2)->get();
            foreach($subcuisines as $subcat){
                $itemlist = Item::with('cuisine_info','subcuisine_info','variation','item_image')->select('item.*',DB::raw('(case when favorite.item_id is null then 0 else 1 end) as is_favorite'),DB::raw('(case when item.price is null then 0 else item.price end) as price'),DB::raw('(case when cart.item_id is null then 0 else 1 end) as is_cart'))
                    ->leftJoin('favorite', function($query) use($user_id) {
                        $query->on('favorite.item_id','=','item.id')
                        ->where('favorite.user_id', '=', $user_id);
                    ...
                    ->groupBy('item.id','cart.item_id')
                    ->where('item.item_status','1')->where('item.is_deleted','2')->where('item.subcuisine_id',$subcat->id)
                    ->orderByDesc('item.id')->get();
                $subcatitems = array();
                foreach ($itemlist as $item) {
                ...
                $data[] = array(
                    "id"=>$subcat->id,
                    "cuisine_id"=>$subcat->cuisine_id,
                    "subcuisine_name"=>$subcat->subcuisine_name,
                    "subcuisine_items"=>$subcatitems,
                );
            }
            return response()->json(["status"=>1,"message"=>trans('messages.success'),'items'=>$data],200);
        }else{
            return response()->json(["status"=>0,"message"=>trans('messages.invalid_cuisine')],200);
        }
    }
    ...
    public function getitemlistobject($itemobject,$user_id)
    {
        ...
        return array(
            ...
            "cuisine_info" => $itemobject->cuisine_info,
            "subcuisine_info" => $itemobject->subcuisine_info,
