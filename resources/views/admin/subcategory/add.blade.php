            ...
            <div class="card border-0">
                <div class="card-body">
                    <div class="form-validation">
                        <form action="{{URL::to('admin/sub-cuisine/store')}}" method="post" enctype="multipart/form-data">
                            @csrf
<div class="row">
                                    <div class="col-md-6">
                            <div class="form-group">
                                <label class="col-form-label" for="">{{trans('labels.subcuisine')}} <span class="text-danger">*</span> </label>
                                    <input type="text" class="form-control" name="name" placeholder="{{trans('labels.subcuisine')}}" value="{{old('name')}}">
                                ...
                                <label class="col-form-label" for="">{{trans('labels.cuisine')}} <span class="text-danger">*</span> </label>
                                    <select class="form-select" name="cuisine">
                                        ...
                                        @foreach ($getcuisine as $cuisine)
                                            <option value="{{$cuisine->id}}" {{old('cuisine') == $cuisine->id ? 'selected' : ''}}>{{$cuisine->cuisine_name}}</option>
                                        @endforeach
                                    </select>
